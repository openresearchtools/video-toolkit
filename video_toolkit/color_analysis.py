...
def build_sampled_levels_gamma_stack(stats: ColorStats) -> tuple[tuple[str, dict[str, object]], ...]:
    """Build a live levels/gamma normalizer from sampled luma percentiles."""

    black_in, mid_in, white_in = _sampled_tone_inputs(stats)
    normalized_mid = _clamp((mid_in - black_in) / max(white_in - black_in, 0.05), 0.08, 0.92)
    target_mid = _sampled_target_midpoint(stats)
    lift_value = _clamp(1.0 + (32.0 - stats.shadow_luma) / 255.0 * 0.18, 0.88, 1.14)
    gamma_value = _clamp(1.0 + (target_mid - normalized_mid) * 0.75, 0.82, 1.22)
    gain_value = _clamp(1.0 + (220.0 - stats.highlight_luma) / 255.0 * 0.14, 0.88, 1.14)
    dynamic_range = stats.luma_p95 - stats.luma_p05
    brightness = _clamp((118.0 - stats.mean_luma) / 255.0 * 0.25, -0.07, 0.08)
    contrast = _clamp((170.0 - dynamic_range) / 170.0 * 12.0, -10.0, 16.0)
    saturation = _clamp(0.5 + (0.32 - stats.mean_saturation) * 0.18, 0.42, 0.58)
    return (
        ("CURVES", {"__curve_points__": {0: _sampled_levels_curve_points(black_in, mid_in, target_mid, white_in)}}),
        (
            "COLOR_BALANCE",
            {
                "color_balance.correction_method": "LIFT_GAMMA_GAIN",
                "color_balance.lift": (lift_value, lift_value, lift_value),
                "color_balance.gamma": (gamma_value, gamma_value, gamma_value),
                "color_balance.gain": (gain_value, gain_value, gain_value),
                "color_multiply": 1.0,
            },
        ),
        ("BRIGHT_CONTRAST", {"bright": brightness, "contrast": contrast}),
        (
            "TONEMAP",
            {
                ...
                "intensity": _clamp(max(stats.luma_p95 - 218.0, 0.0) / 255.0, 0.0, 0.20),
                "contrast": _clamp(abs(52.0 - stats.luma_std) / 255.0, 0.0, 0.18),
                "gamma": _clamp(1.0 + (target_mid - normalized_mid) * 0.22, 0.88, 1.14),
            },
        ),
        ("HUE_CORRECT", {"__hue_correct__": {"saturation": saturation, "value": 0.5}}),
    )
...
def _sampled_tone_inputs(stats: ColorStats) -> tuple[float, float, float]:
    black_in = _clamp(stats.luma_p05 / 255.0, 0.005, 0.35)
    white_in = _clamp(stats.luma_p95 / 255.0, 0.56, 0.995)
    if white_in - black_in < 0.24:
        midpoint = (white_in + black_in) * 0.5
        black_in = _clamp(midpoint - 0.12, 0.005, 0.40)
        white_in = _clamp(midpoint + 0.12, 0.56, 0.995)
    source_mid = stats.midtone_luma or stats.mean_luma
    mid_in = _clamp(source_mid / 255.0, black_in + 0.06, white_in - 0.06)
    return black_in, mid_in, white_in


def _sampled_target_midpoint(stats: ColorStats) -> float:
    return _clamp(0.50 + (118.0 - stats.mean_luma) / 255.0 * 0.08, 0.44, 0.56)


def _sampled_levels_curve_points(
    black_in: float,
    mid_in: float,
    target_mid: float,
    white_in: float,
) -> list[tuple[float, float]]:
    black_out = _clamp(0.02 + max(black_in - 0.20, 0.0) * 0.30, 0.02, 0.16)
    white_out = _clamp(0.98 - max(0.82 - white_in, 0.0) * 0.12, 0.90, 0.98)
    return [
        (0.0, 0.0),
        (black_in, black_out),
        (mid_in, target_mid),

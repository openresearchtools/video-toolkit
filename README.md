...
`shuffleplanes`, `elbg`, `unsharp`, `sobel`, `prewitt`, `kirsch`,
`edgedetect`, `erosion`, `dilation`, `convolution`, `avgblur`, `boxblur`,
`gblur`, `smartblur`, `sab`, `yaepblur`, `dblur`, `hqdn3d`, `nlmeans`,
`bm3d`, `owdenoise`, `vaguedenoiser`, `atadenoise`, `median`, `dedot`,
`deband`, `deblock`, `pseudocolor`, `histeq`, `colorspace`, `colormatrix`,
`setparams`, `setrange`, and `zscale`, then adds editable native Blender VSE
...
- Live one-click Blender stacks: Pro Color Stack, Gamma Grade, Shadow Recovery, Contrast Pop, Warm Grade, Cool Grade, primary color board, log-zone board, ASC CDL finish board, six-vector hue board, secondary skin vector, palette separation board, broadcast-safe finish, match-prep neutralizer, saturation boost/reduce, monochrome, faded film, contrast curves, levels, black/white point correction, luma S-curves, per-channel RGB gamma trims, RGB gamma board, channel mixer balance, opponent color contrast, low/high color correction, independent RGB normalize, hue/saturation/value board, broadcast gamma guard, gray-world neutralizer, RGB LUT trim, selective neutral balance, green/magenta tint repair, shadow/highlight tinting, skin-tone isolation, shadow/highlight balance, vibrance, exposure protection, temperature correction, legal-range clamp, HDR tone compression, and translated FFmpeg color-intent recipes for selective color, colorize, grey-edge balance, pseudocolor, LUT curves, histogram equalization, color hold, and HSV hold.
...
- Native matte/channel tools: translated FFmpeg keying and channel intent as Blender compositor Chroma Matte, Color Matte, Luma Matte, RGBA/chroma channel shift, luma/alpha plane extract, premultiply/straight-alpha conversion, BGR plane shuffle, Channel Matte, Difference Matte, Distance Matte, Keying, Color Spill, Box Mask Alpha, Ellipse Mask Alpha, Double Edge Mask Alpha, and Mask-to-SDF Alpha graphs.
- Native filter/blur tools: translated FFmpeg spatial intent as Blender compositor graphs for ELBG posterize, unsharp, Sobel, Prewitt, Kirsch, edge detect, erosion, dilation, convolution sharpen, average/box/Gaussian/smart/shape-adaptive/edge-preserving/directional blur, deband cleanup, and deblock cleanup.
...
- Native analysis/utility nodes: one-click Blender compositor Levels Monitor, Image Info, Normalize Luma, FFmpeg-translated Histogram, Temporal Histogram, Waveform, Vectorscope, CIE Scope, Data Scope, Oscilloscope, and Signal Stats diagnostic graphs, Image Coordinates, Relative to Pixel, Split Compare, Switch Compare, Cryptomatte, Cryptomatte V2, Map UV, Plane Track Deform, Z Combine, Sequencer Strip Info, Scene Time, Time, and Track Position graph helpers.
...
- Native denoise/cleanup tools: translated FFmpeg hqdn3d, nlmeans, BM3D, owdenoise, vaguedenoiser, atadenoise, median, and dedot intent as Blender compositor Denoise and Despeckle graphs, with temporal-only behavior represented by editable spatial node strength where Blender has no native temporal node.
...
- Motion and output: 2x Lanczos upscale, 1080p normalize scale, 60 fps interpolation, temporal smoothing, plus native Blender compositor resize/reframe and motion-geometry node graphs.
...
Open Research Video Toolkit is licensed under the GNU General Public License, version 3 or later (`GPL-3.0-or-later`).

...
- **Native compositor node stacks**: one-click static, sampled, diagnostic, palette-identity, reference-matched, animated timeline-matched, FFmpeg-translated, and keyframed lighting-normalizer color/restoration graphs plus a full native video-finishing node library created from the active movie strip using Blender's compositor nodes.

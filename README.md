...
`datascope`, `oscilloscope`, `pixscope`, `signalstats`, `colordetect`, `identity`,
...
- Native analysis/utility nodes: one-click Blender compositor Levels Monitor, Image Info, Normalize Luma, FFmpeg-translated Histogram, Temporal Histogram, Waveform, Vectorscope, CIE Scope, Data Scope, Oscilloscope, Pixel Scope, Signal Stats, Color Detect, Identity, SSIM, PSNR, XPSNR, Correlation, MSAD, and Cross-Correlation reference-quality diagnostic graphs, Image Coordinates, Relative to Pixel, Split Compare, Switch Compare, Cryptomatte, Cryptomatte V2, Map UV, Plane Track Deform, Z Combine, Sequencer Strip Info, Scene Time, Time, and Track Position graph helpers.

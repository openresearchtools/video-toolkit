...
Current real-video evidence from Blender 5.2 Beta is under `tests/output/current_real_mp4_parameter_visual_matrix_after_native_expansion_fixed2/` and `tests/output/current_real_mp4_compositor_visual_matrix_after_native_expansion/`: the parameter matrix passed 5,738 rendered parameter checks with 0 failures, converted 3,883 compositor parameter PNGs, and wrote 575 contact sheets; the compositor visual matrix rendered 356 tool outputs with 0 failures and 30 before/after review pages. The four compositor graph-only notes are Blender utility graphs that do not emit a renderable RGB Output File image in headless Blender 5.2.

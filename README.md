...
`procamp_vaapi`, `tonemap_opencl`, `tonemap_vaapi`,
`palettegen`, `paletteuse`, `amplify`,
`chromakey`, `chromakey_cuda`, `colorkey`, `colorkey_opencl`, `hsvkey`, `lumakey`, `despill`, `backgroundkey`, `rgbashift`, `chromashift`, `chromaber_vulkan`,
`threshold`, `maskedthreshold`, `blend`, `blend_vulkan`, `tblend`, `lut2`, `tlut2`, `maskedmerge`,
`mergeplanes`,
`alphaextract`, `alphamerge`, `extractplanes`, `premultiply`, `unpremultiply`,
`shuffleplanes`, `elbg`, `unsharp`, `cas`, `sobel`, `prewitt`, `kirsch`,
`edgedetect`, `erosion`, `dilation`, `convolution`, `convolution_opencl`, `fftfilt`, `avgblur`,
...
`deblock`, `deflicker`, `bwdif`, `yadif`, `deshake`, `vidstabdetect`,
`vidstabtransform`, `tmix`, `fps`, `framerate`, `minterpolate`,
`histogram`, `thistogram`, `waveform`, `vectorscope`, `ciescope`,
`datascope`, `oscilloscope`, `pixscope`, `signalstats`, `colordetect`,
`blackdetect`, `blackdetect_vulkan`, `blackframe`, `blockdetect`, `blurdetect`,
`cropdetect`, `bbox`, `bitplanenoise`, `freezedetect`, `scdet`, `scdet_vulkan`,
`vfrdet`, `idet`, `identity`,
`ssim`, `psnr`, `xpsnr`, `corr`, `msad`, `xcorrelate`, `pseudocolor`,
`geq`, `lut`, `lutrgb`, `lutyuv`, `lut1d`, `lut3d`, `haldclut`,
`colormap`, `histeq`, `midequalizer`,
`tmidequalizer`, `colorspace`, `colorspace_cuda`, `colormatrix`, `setparams`, `setrange`, and
...
- Live one-click Blender stacks: Pro Color Stack, Gamma Grade, Shadow Recovery, Contrast Pop, Warm Grade, Cool Grade, primary color board, log-zone board, ASC CDL finish board, six-vector hue board, secondary skin vector, palette separation board, broadcast-safe finish, match-prep neutralizer, saturation boost/reduce, monochrome, faded film, contrast curves, levels, black/white point correction, luma S-curves, per-channel RGB gamma trims, RGB gamma board, channel mixer balance, opponent color contrast, low/high color correction, independent RGB normalize, hue/saturation/value board, broadcast gamma guard, gray-world neutralizer, RGB LUT trim, selective neutral balance, GEQ RGB math, 1D LUT film curve, 3D LUT scene look, Hald CLUT display match, colormap palette match, FFmpeg palettegen/paletteuse previews, temporal-change amplify preview, midway/temporal midway equalization, green/magenta tint repair, shadow/highlight tinting, skin-tone isolation, shadow/highlight balance, vibrance, exposure protection, temperature correction, legal-range clamp, HDR tone compression, and translated FFmpeg color-intent recipes for selective color, colorize, grey-edge balance, pseudocolor, LUT curves, histogram equalization, color hold, and HSV hold.
- Native Blender primitives: VSE Brightness/Contrast, Lift/Gamma/Gain, ASC CDL Offset/Power/Slope, R/D Photoreceptor Tone Map, Rh Simple Tone Map, Curves, Hue Correct, White Balance, Mask, plus one-click compositor primitive graphs for Exposure, Color Correction, Hue/Saturation/Value, Invert, Posterize, Premul Key, Convert Color Space, Convert to Display, Set Alpha, Alpha Over, RGB/HSV/HSL/YUV/YCbCr 601/709/JFIF color-model boards, and the combined all-compositor-color-primitives graph.
- Native matte/channel tools: translated FFmpeg keying, channel, and composite intent as Blender compositor Chroma Matte, Color Matte, Luma Matte, Color Spill despill, Background Key Difference Matte/Set Alpha, threshold/masked-threshold luma matte graphs, Alpha Over blend/tblend/lut2/tlut2 composites, maskedmerge luma-matte composites, mergeplanes channel routing, RGBA/chroma channel shift, chromaber_vulkan chromatic-aberration red/blue channel offsets, luma/alpha plane extract, alphamerge RGB-to-BW luma matte into Set Alpha, premultiply/straight-alpha conversion, BGR plane shuffle, Channel Matte, Difference Matte, Distance Matte, Keying, Color Spill, Box Mask Alpha, Ellipse Mask Alpha, Double Edge Mask Alpha, and Mask-to-SDF Alpha graphs.
- Native filter/blur tools: translated FFmpeg spatial intent as Blender compositor graphs for ELBG posterize, unsharp, CAS sharpen, FFT detail filtering, Sobel/OpenCL Sobel, Prewitt/OpenCL Prewitt, Roberts/OpenCL Roberts preview, Kirsch, edge detect, erosion/OpenCL erosion, dilation/OpenCL dilation, convolution sharpen, average/OpenCL/Vulkan average blur, box/OpenCL box blur, Gaussian/Vulkan Gaussian blur, variable blur, bilateral/CUDA bilateral, smart/shape-adaptive/edge-preserving/directional blur, deband cleanup, gradfun anti-banding, and deblock cleanup.
...
- Native analysis/utility nodes: one-click Blender compositor Levels Monitor, Image Info, Normalize Luma, FFmpeg-translated Histogram, Temporal Histogram, Waveform, Vectorscope, CIE Scope, Data Scope, Oscilloscope, Pixel Scope, Show Palette, Thumbnail/CUDA Thumbnail, Signal Stats, Color Detect, Entropy, Black Segment Detect, Vulkan Black Detect, Black Frame Detect, Block Artifact Detect, Blur Detect, Crop Detect, Bounding Box Detect, Bit Plane Noise Detect, Freeze Detect, Scene Change Detect, Vulkan Scene Detect, Variable Frame Rate Detect, Interlace Detect, Identity, SSIM, PSNR, XPSNR, VIF, VMAF Motion, Correlation, MSAD, and Cross-Correlation reference-quality diagnostic graphs, Image Coordinates, Relative to Pixel, Split Compare, Switch Compare, Cryptomatte, Cryptomatte V2, Map UV, Plane Track Deform, Z Combine, Sequencer Strip Info, Scene Time, Time, and Track Position graph helpers.
- Native source/output nodes: selected-strip Movie Clip source, Viewer tap, Output File tap, image/render-layer/normal/group placeholders, RGB overlay, Blank Image overlay, editable text overlay, Bokeh Image blur, Switch View graph helpers, and FFmpeg source/generator previews (`allrgb`, `allyuv`, `cellauto`, `color`, `colorchart`, `colorspectrum`, `gradients`, `haldclutsrc`, `life`, `mandelbrot`, `pal75bars`, `pal100bars`, `rgbtestsrc`, `smptebars`, `smptehdbars`, `testsrc`, `testsrc2`, `yuvtestsrc`, `zoneplate`, and related generators) mapped to editable Blank Image/Text source graphlets with original generator metadata preserved.
- Native filter/denoise/cleanup tools: translated FFmpeg sharpen, edge, kernel, OpenCL/Vulkan blur, VAAPI sharpness/denoise, hqdn3d, chromanr, fftdnoiz, nlmeans/OpenCL NLMeans/Vulkan NLMeans, DCT denoise, BM3D, owdenoise, vaguedenoiser, atadenoise, median, temporal median, xmedian, and dedot intent as Blender compositor Filter, Blur, Denoise, Bilateral Blur, and Despeckle graphs, with temporal/frequency/hardware-only behavior represented by editable native node strength where Blender has no direct temporal or hardware execution node.
- Native temporal/motion previews: translated FFmpeg deflicker, bwdif/CUDA BWDIF/Vulkan BWDIF, yadif/CUDA YADIF, ESTDIF, W3FDIF, QSV/VAAPI deinterlace previews, field/fieldhint/fieldmatch/fieldorder/setfield/separatefields/repeatfields, telecine/detelecine, decimate/mpdecimate, MCDeint, NNEDI, deshake/OpenCL deshake, vidstabdetect/vidstabtransform, tmix, fps, framerate, and minterpolate intent as Blender Tone Map, luma monitor, Anti-Aliasing, Blur, Stabilize, Transform, Alpha Over, and Directional Blur graphlets with original timing/stabilization metadata preserved for exact rendered fallback output.
- Native geometry/lens tools: translated FFmpeg scale/CUDA scale/QSV scale/VAAPI scale/Vulkan scale, fillborders, floodfill, untile, v360, crop, rotate, transpose/OpenCL transpose/VAAPI transpose/Vulkan transpose, horizontal/vertical/Vulkan both-axis flip, and lenscorrection intent plus direct Blender Stabilize, Movie Distortion, Corner Pin, Displace, Transform, and Translate compositor graphs.
- Blender VSE modifiers: Brightness/Contrast, Color Balance, Curves, Hue Correct, Mask, Tone Map, White Balance.
- Blender compositor nodes: active-strip Movie Clip source, the **Video Effects** sidebar browser `Nodes` action plus the **Compositor Color Recipes** menu for every supported Blender-native catalog color recipe, sampled **Recommended Recipe Mix Nodes**, one-click **Pro Color Workflow** recipe and Color Management graphs, one-click **FFmpeg Color Workflow** translated color graphs, sampled dynamic **Color Board** graphs, reference **Color Board** graphs, a one-click **All Recipe Nodes** action that generates every supported catalog recipe graph, a **Catalog Coverage Report** text datablock that lists native/compositor/rendered fallback coverage plus FFmpeg-to-Blender translation coverage, connected native color-room graph, sampled Color Management graph for exposure/gamma/white balance/view curves/display conversion, sampled real-frame color graph, diagnostic recommendation graph, palette-identity color graph, reference-matched color graph, animated color-timeline match graph, FFmpeg-translated color graph, keyframed lighting-normalizer graph, individual one-click compositor primitives for Exposure, Brightness/Contrast, Color Balance, Color Correction, RGB Curves, Hue/Saturation/Value, Hue Correct, Tone Map, Invert, Posterize, Premul Key, Normalize, RGB to BW, Separate Color, and Combine Color, display conversion, channel split/combine boards, luma/normalize monitor nodes, Directional Blur, reusable node groups, Levels, Viewer, Output File, and a full native node library organized by color, restoration, transform, matte, input/output, and utility groups.
...
- Blender-native Color Management tools: sampled real-frame scene profile plus AgX balanced, AgX punch, Filmic soft, Standard video, warm review, and view-curve contrast presets; one-click sidecar metadata pipelines for FFmpeg `colorspace`, `colorspace_cuda`, `colormatrix`, `setparams`, `setrange`, and `zscale` intent that apply Blender Sequencer color-management metadata and create native compositor review graphs.
- FFmpeg-to-Blender translation: supported FFmpeg color intent (`eq`, `hue`, `huesaturation`, `colorchannelmixer`, `curves`, `colorlevels`, `colorbalance`, `normalize`, `colorcorrect`, `colorcontrast`, `selectivecolor`, `monochrome`, `colorize`, `grayworld`, `greyedge`, `negate`, `chromahold`, `colorhold`, `hsvhold`, `pseudocolor`, `geq`, simple `lut`/`lutrgb`/`lutyuv`, file/CLUT look intent from `lut1d`/`lut3d`/`haldclut`, `colormap`, `palettegen`, `paletteuse`, `amplify`, `histeq`, `midequalizer`, `tmidequalizer`, `vibrance`, `exposure`, `colortemperature`, `limiter`, `tonemap`, `procamp_vaapi`, `tonemap_opencl`, `tonemap_vaapi`), compositor-only key/matte/composite intent (`chromakey`, `chromakey_cuda`, `colorkey`, `colorkey_opencl`, `hsvkey`, `lumakey`, `despill`, `backgroundkey`, `threshold`, `maskedthreshold`, `blend`, `blend_vulkan`, `tblend`, `lut2`, `tlut2`, `maskedmerge`, `mergeplanes`), compositor channel/plane/alpha intent (`rgbashift`, `chromashift`, `chromaber_vulkan`, `alphaextract`, `alphamerge`, `extractplanes`, `premultiply`, `unpremultiply`, `shuffleplanes`), compositor posterize/sharpen/edge/kernel/blur/geometry/lens/restoration/matte-cleanup intent (`elbg`, `unsharp`, `unsharp_opencl`, `cas`, `sobel`, `sobel_opencl`, `prewitt`, `prewitt_opencl`, `roberts`, `roberts_opencl`, `kirsch`, `edgedetect`, `erosion`, `erosion_opencl`, `dilation`, `dilation_opencl`, `convolution`, `convolution_opencl`, `fftfilt`, `avgblur`, `avgblur_opencl`, `avgblur_vulkan`, `boxblur`, `boxblur_opencl`, `gblur`, `gblur_vulkan`, `varblur`, `bilateral`, `bilateral_cuda`, `smartblur`, `sab`, `yaepblur`, `dblur`, `scale`, `scale_cuda`, `scale_qsv`, `scale_vaapi`, `scale_vulkan`, `fillborders`, `floodfill`, `untile`, `v360`, `crop`, `rotate`, `transpose`, `transpose_opencl`, `transpose_vaapi`, `transpose_vulkan`, `hflip`, `hflip_vulkan`, `vflip`, `vflip_vulkan`, `flip_vulkan`, `lenscorrection`, `xbr`, `hqdn3d`, `chromanr`, `denoise_vaapi`, `fftdnoiz`, `nlmeans`, `nlmeans_opencl`, `nlmeans_vulkan`, `dctdnoiz`, `bm3d`, `owdenoise`, `vaguedenoiser`, `atadenoise`, `median`, `tmedian`, `xmedian`, `dedot`, `deband`, `gradfun`, `sharpness_vaapi`, `deblock`), compositor temporal/motion restoration intent (`deflicker`, `bwdif`, `bwdif_cuda`, `bwdif_vulkan`, `yadif`, `yadif_cuda`, `estdif`, `w3fdif`, `deinterlace_qsv`, `deinterlace_vaapi`, `field`, `fieldhint`, `fieldmatch`, `fieldorder`, `setfield`, `separatefields`, `repeatfields`, `telecine`, `detelecine`, `decimate`, `mpdecimate`, `mcdeint`, `nnedi`, `deshake`, `deshake_opencl`, `vidstabdetect`, `vidstabtransform`, `tmix`, `fps`, `framerate`, `minterpolate`), compositor diagnostic/scope/reference-quality intent (`histogram`, `thistogram`, `waveform`, `vectorscope`, `ciescope`, `datascope`, `oscilloscope`, `pixscope`, `showpalette`, `thumbnail`, `thumbnail_cuda`, `signalstats`, `colordetect`, `entropy`, `blackdetect`, `blackdetect_vulkan`, `blackframe`, `blockdetect`, `blurdetect`, `cropdetect`, `bbox`, `bitplanenoise`, `freezedetect`, `scdet`, `scdet_vulkan`, `vfrdet`, `idet`, `identity`, `ssim`, `psnr`, `xpsnr`, `vif`, `vmafmotion`, `corr`, `msad`, `xcorrelate`), source/generator intent (`allrgb`, `allyuv`, `cellauto`, `color`, `colorchart`, `colorspectrum`, `gradients`, `haldclutsrc`, `life`, `mandelbrot`, `mptestsrc`, `nullsrc`, `pal75bars`, `pal100bars`, `perlin`, `random`, `rgbtestsrc`, `sierpinski`, `smptebars`, `smptehdbars`, `testsrc`, `testsrc2`, `yuvtestsrc`, `zoneplate`), and color metadata (`colorspace`, `colorspace_cuda`, `colormatrix`, `setparams`, `setrange`, `zscale`) is converted into native live Blender VSE modifier stacks, direct Blender compositor Brightness/Contrast, Hue/Saturation, Exposure, Color Correction, Color Balance, RGB Curves, Hue Correct, Tone Map, Invert, matte/color/channel/plane/alpha-conversion/composite/convolution/blur/transform/lens/restoration/detail/temporal-preview/diagnostic scope/detector/reference-difference/reference-quality/source-overlay nodes, and Blender color-management settings from the **Native Color Chain Translation**, **FFmpeg Color Workflow**, and **Translated** compositor controls; exact frame-neighborhood temporal output still stays in rendered restoration, while native preview graphlets expose editable Blender controls where Blender has no temporal node.
- Restoration: deflicker, lighting normalizer, denoise, sharpen restoration, deinterlace, quick deshake, two-pass vidstab stabilization, plus one-click native Blender compositor restoration graphs for deflicker preview, bwdif/yadif deinterlace preview, deshake/vidstab preview, temporal mix preview, Denoise/Despeckle/Bilateral Blur/Anti-Aliasing, sharpen cleanup, and lens repair.
- Motion and output: 2x Lanczos upscale, 1080p normalize scale, 60 fps interpolation, temporal smoothing, plus native Blender compositor XBR-style upscale, FPS/framerate/minterpolate motion previews, resize/reframe, and motion-geometry node graphs.
...
`scripts/end_user_parameter_visual_matrix.py` applies every live Blender tool and compositor-capable sidecar tool to a real selected movie strip, mutates every discovered editable VSE modifier property and compositor socket through Blender's API, renders before/after PNG evidence, records visible pixel deltas, and writes contact sheets plus `parameter_visual_report.json` for end-user inspection.

Current real-video evidence from Blender 5.2 Beta is under `tests/output/current_real_mp4_parameter_visual_matrix_after_source_generators/`, `tests/output/current_real_mp4_compositor_visual_matrix_after_source_generators/`, and `tests/output/current_real_mp4_visual_inspection_after_source_generators/`: the parameter matrix passed 5,954 of 5,995 rendered parameter checks with 0 failures, converted 4,126 compositor parameter PNGs, and wrote 596 contact sheets (219 live-parameter sheets and 377 compositor-socket sheets); the compositor visual matrix rendered 383 tool outputs from 387 compositor tools with 0 failures, converted 383 PNGs, and wrote 32 before/after review pages. The four compositor graph-only notes are Blender utility graphs that do not emit a renderable RGB Output File image in headless Blender 5.2.

`scripts/end_user_color_pipeline_matrix.py` focuses on the FFmpeg color metadata sidecar tools. It opens a real MP4 in Blender, applies each native color pipeline tool through `video_toolkit.apply_filter`, verifies the selected strip, scene color-management metadata, and stamped compositor-node metadata, renders one compositor frame per tool, and writes a compact before/after contact sheet under `tests/output/color_pipeline_matrix/`.
